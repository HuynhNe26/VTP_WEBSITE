class AuthUser {
    ...
    static saveToken(token) {
        ...
        sessionStorage.setItem('token', JSON.stringify(token)); // Sửa key thành 'token'
    }
    ...
    static getIdUser() {
        ...
        try {
            const payload = this.decodeToken(token);
            return payload.id_user || payload.sub || null; // Linh hoạt với key id_user
        } catch (error) {
            console.error('Error decoding token for id_user:', error);
            return null;
        }
    }
