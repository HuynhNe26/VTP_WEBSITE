...
const ManageOrderAll = () => {
  ...
  const handleConfirm = async (orderId) => {
    try {
      ...
      if (response.ok) {
        // Cập nhật trạng thái trong danh sách đơn hàng trên giao diện
        setOrders(orders.map(order =>
          order.id_order === orderId ? { ...order, status: 'Đã giao' } : order
        ));

        // Lấy thông tin đơn hàng để tìm id_user
        const order = orders.find(order => order.id_order === orderId);
        if (order && order.id_user) {
          // Cập nhật check_buy_success của người dùng
  ...
  // Hàm cập nhật check_buy_success của người dùng
  const updateUserBuySuccess = async (userId) => {
    try {
      const response = await fetch(`http://localhost:5000/users/${userId}/buy-success`, {
        ...
        body: JSON.stringify({ check_buy_success: 1 }), // Tăng giá trị (có thể thay đổi logic nếu cần)
      });
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      ...
      case 'Đã giao': return styles.statusCompleted; // Cập nhật để hiển thị đúng trạng thái "Đã giao"
